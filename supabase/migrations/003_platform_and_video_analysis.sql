-- 003 platform layer + bareback_runs for barebackbronc-mobile-app
...
-- Discipline run log for bareback riding.
create table if not exists public.bareback_runs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  ...
  horse_name text,
  rigging_type text,
  rigging_position text,
  free_arm_discipline text,
  body_position text,
...
create index if not exists bareback_runs_user_idx on public.bareback_runs (user_id, created_at desc);

alter table public.bareback_runs enable row level security;

drop policy if exists "bareback_runs_owner_all" on public.bareback_runs;
create policy "bareback_runs_owner_all" on public.bareback_runs

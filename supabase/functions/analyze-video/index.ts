...
// analyze-video edge function for bareback riding.
//
...
const ANALYSIS_SCHEMA = {
  ...
  properties: {
    ...
    phases: {
      type: "object",
      additionalProperties: false,
      required: ["mark_out", "spur_stroke", "free_arm", "body_position"],
      properties: {
          mark_out: { type: "string" },
          spur_stroke: { type: "string" },
          free_arm: { type: "string" },
...
const SYSTEM_PROMPT =
  "You are an expert bareback riding coach. Analyze the athlete's run from the supplied frames. " +
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: SYSTEM_PROMPT + " JSON schema: " + JSON.stringify(ANALYSIS_SCHEMA) },
          {
            role: "user",
            content: [
              { type: "text", text: "Analyze this bareback riding run." },
              ...images,
            ],
          },
        ],
    ...
    if (supabaseUrl && serviceKey && user_id) {
      ...
      await admin.from("video_analysis_results").insert({
        user_id,
        event_type: body.event_type ?? "barebackbronc",
        video_url,
